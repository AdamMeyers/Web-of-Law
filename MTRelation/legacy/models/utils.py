...
class GRUBiRNN(nn.Module):
    def __init__(self, embedding, dim_hid, num_rnn_layer):
        ...
        self.l_birnn = nn.GRU(embedding.shape[1], dim_hid, num_rnn_layer, bidirectional=True)
        self.vocab_size = embedding.shape[0]

    def forward(self, x):
        seq_lens = np.sum(x != self.vocab_size - 1, axis=1)
        perm_indices = np.argsort(-1 * seq_lens)
        x = x[perm_indices].transpose()
        seq_lens = seq_lens[perm_indices]

        x = create_var(torch.from_numpy(x))
        x = pack_padded_sequence(self.l_emb(x), seq_lens)
        _, hn = self.l_birnn(x)
...
class LSTMBiRNN(nn.Module):
    def __init__(self, embedding, dim_hid, num_rnn_layer):
        ...
        self.l_emb = nn.Embedding(embedding.shape[0], embedding.shape[1], sparse=True)
        self.l_emb.weight.data.copy_(torch.from_numpy(embedding))
        self.l_birnn = nn.LSTM(embedding.shape[1], dim_hid, num_rnn_layer, bidirectional=True)
        # self.l_birnn = nn.GRU(embedding.shape[1], dim_hid, num_rnn_layer, bidirectional=True)
        self.vocab_size = embedding.shape[0]
...
class GRUBiRNNWord(nn.Module):
    def __init__(self, embedding, dim_hid, num_rnn_layer):
        ...
        self.l_emb = nn.Embedding(embedding.shape[0], embedding.shape[1], sparse=True)
        self.l_emb.weight.data.copy_(torch.from_numpy(embedding))
        self.l_birnn = nn.GRU(embedding.shape[1], dim_hid, num_rnn_layer, batch_first=True, bidirectional=True)
        self.vocab_size = embedding.shape[0]
    ...
    def forward(self, x):
        ...
        x = create_var(torch.from_numpy(x))
        x = pack_padded_sequence(self.l_emb(x), seq_lens, batch_first=True)
        x, _ = self.l_birnn(x)
        ...
        if torch.cuda.is_available():
            perm_indices_rev = perm_indices_rev.cuda()
        return x[perm_indices_rev], seq_lens[perm_indices_rev]
