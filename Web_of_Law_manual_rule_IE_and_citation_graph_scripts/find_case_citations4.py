...
pre_citation_words = ['see','also','cf','compare','later']
...
def possible_abbrev(word):
    ...
    if word.istitle() and ((len(word)==1) or (word.lower() in ['assoc', 'asst', 'capt', 'cdr', 'cmdr', 'col', 'dr', 'dr', 'exec', \
...
def end_of_sentence_heuristic (word,line,next_position):
    ...
    if end_of_sent_pattern and not (possible_abbrev(word)):
        rest_of_line = line[end_of_sent_pattern.end():]
        if re.search('^ *[a-z]',rest_of_line):
    ...
    for span_start,span_end in spans:
        if (out['start']>=span_start):
            if out['end'] <=span_end:
                ...
                output = []
                if new_phrase1:
                    output.append([new_phrase1,new_sequence1])
                if new_phrase2:
                    output.append([new_phrase2,new_sequence2])
                if len(output)>0:
                    return(output,False)
                else:
                    return(False,False)
...
def ok_role_phrase(out,sequence,spans,string,line,offset,words):
    ...
    if fail:
        ...
        for new_out2,new_sequence2,new_words2 in triples:
            ...
            if good:
                new_phrases.append([new_out2,new_sequence2])
            elif new_phrases2:
    ...
    if new_phrases and (len(new_phrases)>0):
        ...
        if good:
            return([[new_out,new_sequence]],False)
        else:
            return(False,False)
    else:
...
def find_phrase_type_from_sequence(sequence):
    if 'LEGAL_ROLE' in sequence:
        ...
        for POS in POS_dict[word.lower()]:
            if POS in ['NOUN','ADJECTIVE']:
                keep = True
                         ...
                         'AUX', 'CCONJ', 'DET', 'ADVPART', 'SCOPE','DISCOURSE']:
                toss = True
...
def get_role_phrases(line,spans,offset,file_id,citation_dictionary,individual_spans,line_number):
    ...
    while match:
        ...
        if not match:
            ...
            if ('start' in out) and (len(sequence)>0) and (entry_type in ['LEGAL_ROLE','FAMILY','PROFESSION','ORGANIZATION']) and \
              ...
              (((entry_type == 'ORGANIZATION') and (('ORGANIZATION' in sequence) or  ('PLURAL_ORGANIZATION' in sequence))) or \
               ...
               ((entry_type == 'PROFESSIONAL') and (('PROFESSIONAL' in sequence) or ('PLURAL_PROFESSIONAL' in sequence) or \
                ...
                if new_phrases:
                    for phrase,seq in new_phrases:
                        abort_phrase = False
                        if 'name_with_infixed_role' in phrase:
                            phrase['phrase_type'] = 'NAME'
                            plural = False
                        else:
                            value,plural = find_phrase_type_from_sequence(seq)
                            if value:
                    ...
                    value,plural = find_phrase_type_from_sequence(sequence)
                    if value:
            ...
            add_on = True
            start = match.end()
        elif (word.lower() in not_uppercase_part_of_org_names) and (len(sequence))>0:
            if len(sequence)>0:
                add_on = True
                sequence.append('FILLER')
                words.append(word)
          ...
          (re.search('^[\.]?[ \t]{2,}',line[match.end():]) or (not re.search('[a-zA-Z]',line[match.end():]))):
            ...
            if len(sequence)==0 and ok_non_vocab_word_start_sequence(word,start_sentence):
                out['start']=match.start()+offset
                add_on = True
            elif len(sequence)>0:
                add_on = True
        ...
        if not match:
          ...
          ((not ',' in line[previous_start:match.start()]) or (word.lower() in org_ending_words) or (word.lower() in person_ending_words)):
            ...
            if 'start' in out:
                ...
                good = False
            if good:
                phrase_type,plural = find_phrase_type_from_sequence(sequence)
                if phrase_type:
                ...
                for phrase,seq in new_phrases:
                    if 'name_with_infixed_role' in phrase:
                        ...
                        phrase_type,plural = find_phrase_type_from_sequence(seq)
                    if phrase_type:
            ...
            if ('start' in out):
                ...
                if (not re.search('[a-zA-Z]',line[match.end():])) or end_of_sentence:
                    ...
                    if match.group(0).lower() in relational_dict:
                        phrase_type,plural = find_phrase_type_from_sequence([merge_relational_entries(match.group(0),relational_dict[match.group(0).lower()])])
                    else:
                        phrase_type = False
    ...
    final_output=[]
    for record in output:
        if not [record['start'],record['end']] in individual_spans:
            ## prevents duplicates
            add_citation_id(record,file_id,citation_dictionary)
    ...
    for entity in entities:
        ...
        if (not last_entity):
            ...
            if last_entity_small and (last_entity_small['end'] < entity['start']):
                ...
                gram_type,preposition = get_gram_type_from_inbetween(inbetween,entity_type,last_type,line_number,line,last_entity_small,offset)
                family = False
                theme = False
                relation_type = False
                legal_role = False
                ...
                if not gram_type:
                    relation = False
                if (gram_type == 'possessive') and (last_type == 'FAMILY') and (current_type == 'NAME'):
                    family = last_entity_small
                    theme = entity
                    relation_type = 'asymmetric'
                elif (last_type == 'LEGAL_ROLE') and (current_type in ['ORGANIZATION','PROFESSION','FAMILY','NAME']) and (gram_type in ['pre_nom']):
                    legal_role = last_entity_small
                    theme = entity
                    relation_type = 'asymmetric'
                elif (current_type == 'LEGAL_ROLE') and (last_type in ['ORGANIZATION','PROFESSION','FAMILY','NAME']) \
                          and (gram_type in ['apposition','as_predication','preposition']):
                    legal_role = entity
                    theme = last_entity_small
                    relation_type = 'asymmetric'
                elif (last_type == 'PROFESSION') and (current_type == 'NAME') and (gram_type in ['pre_nom']):
                    profession = last_entity_small
                    theme = entity
                    relation_type = 'asymmetric'
                elif (current_type == 'PROFESSION') and (last_type == 'NAME') \
                  and (gram_type in ['apposition','as_predication','preposition']):
                    profession = entity
                    theme = last_entity_small
                    relation_type = 'asymmetric'                    
                elif (last_type == 'FAMILY') and (current_type == 'NAME') and (gram_type in ['pre_nom']):
                    family = last_entity_small
                    theme = entity
                    relation_type = 'partly_asymmetric'
                elif (current_type == 'FAMILY') and (last_type == 'NAME')\
                  and (gram_type in ['apposition','as_predication','preposition']):
                    family = entity
                    theme = last_entity_small
                    relation_type = 'partly_asymmetric'
            ...
            gram_type,preposition = get_gram_type_from_inbetween(inbetween,current_type,last_type,line_number,line,last_entity,offset)
            ...
            if not gram_type:
                ...
                for partner,gram_type,preposition in partner_tuples:
                    ...
                    if (gram_type == 'conj'):
                        ...
                        if (last_type == 'FAMILY') and (current_type == 'NAME'):
                            family = partner
                            theme = entity
                            relation_type = 'asymmetric'
                        else:
                    ...
                    if (len(relations)>0) and relation:
                        ...
                        if ('theme' in relation) and (relation['theme']==last_entity) and ('theme' in last_relation) and \
                          (get_type_from_entity(last_relation['theme'])=='NAME') and ('legal_role' in relation) and (last_type == 'PROFESSION'):
...
def merge_spans2(spans,types,line,offset):
    ...
    for span in spans:
        if not last_span:
            ...
            pass
        elif last_span and (num > 0) and (last_type in ['NAME','ORGANIZATION']) \
    ...
    with open(txt_file) as instream:
        ...
        for line in instream:
            ...
            if (not garbage) and (not line_combo):
                ...
                parties = get_party_names_and_roles_from_cases(out3,file_id,citation_dictionary)
            else:
                parties = False
            if (not garbage) and (not line_combo):
            ...
            if (not garbage) and (not line_combo):
                out5 = get_role_phrases(line,spans2,offset,file_id,citation_dictionary,spans,line_number)
