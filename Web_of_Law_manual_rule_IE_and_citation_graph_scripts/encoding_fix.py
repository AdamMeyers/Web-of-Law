...
##########################
#Changes 11/29/2018
#Shortened main dict. name
#Added list comprehensions
#Combined some lines
#Added spaces for readability
###########################

cp1252_to_utf8 = {  #cp1252_to_utf8_chars_decoding_table
	'0x28': '', # ='('  #No other additions or changes to this dict.
...
def decode_json_files_in_directory(indirectory, outdirectory):
	target_fields=['html_lawbox', 'html_columbia', 'html_with_citations']
	all_files=list(os.listdir(indirectory))
	for json_file in [any_file for any_file in all_files if any_file.endswith('.json')]:
		with open(file_name_append(indirectory,json_file),'r') as instream:
			json_obj = json.loads(instream.read())
			for field in[anyfield for anyfield in target_fields if (anyfield in json_obj and json_obj[anyfield])]:
				tokens = json_obj[field].split(' ')
				for i in range(0,len(tokens)):
					if (len(tokens[i]) == 1):
						if (str(hex(ord(tokens[i]))) in cp1252_to_utf8.keys()):
							tokens[i]= cp1252_to_utf8[str(hex(ord(tokens[i])))]
					elif len(tokens[i])>1:
						token = tokens[i]
						if (str(hex(ord(token[-1]))) in cp1252_to_utf8):
							tokens[i]=token[:-1]+cp1252_to_utf8[str(hex(ord(token[-1])))]
                         #spaces for readability
						if (len(token)==3) and (token[0] in '({[') and (token[2] in ')}]') and (str(hex(ord(token[1]))) in cp1252_to_utf8):
							tokens[i]=token[0]+cp1252_to_utf8[str(hex(ord(token[1])))]+token[2]
						elif '<' in token:
							position = token.find('<')
							if (position>0) and (str(hex(ord(token[position-1]))) in cp1252_to_utf8):
								token = token[:position-1]+cp1252_to_utf8[str(hex(ord(token[position-1])))]+token[position:]
							tokens[i]=token

						if str(hex(ord(token[0]))) in cp1252_to_utf8:
							tokens[i]=cp1252_to_utf8[str(hex(ord(token[0])))]+token[1:]
						elif '>' in token:
							position = token.find('>')
							if (position<len(token)-1) and (str(hex(ord(token[position+1]))) in cp1252_to_utf8):
								token = token[:position]+cp1252_to_utf8[str(hex(ord(token[position+1])))]+token[position+1:]
								tokens[i]=token

						if chr(151) in token: #chr(151)	= ——
