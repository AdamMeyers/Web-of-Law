...
def main(path_data='../data/wikipedia/',
         file_name='europarl-v7.es-en.en.tok.lower.numbered',
         max_len=50):
    ...
    for suffix in ['.dev']:
        ...
        with open(path_data + file_name + suffix + '.' + str(max_len), 'w') as fout:
            with open(path_data + file_name + suffix, 'r') as fin:
                ...
                for curr_idx, line in enumerate(fin):
                    ...
                    while len(sent_proc) > max_len:
                        fout.write(line_num + '\t' + ' '.join(sent[:max_len]) + '\n')
                        sent = sent[max_len:]
        ...
        print 'Dumping ...'
        cPickle.dump([data_X, data_Y], open(path_data + file_name + suffix + '.' + str(max_len) + '.dat', 'w'))
        print 'Done\n'

        print 'Shape of data:', data_X.shape, '\n'
